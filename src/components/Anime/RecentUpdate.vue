  ...
  <CommenItem class="update-box" title="最近更新">
    <ul :class="showAll ? 'auto-height' : ''">
      <li v-for="item in list" :key="item.id">
        <div class="left-pic">
          <a :href="`/play/${item.id}?ep=${item.new_ep}`" target="_blank">
            <img :src="item.small_img" />
          </a>
        </div>
        <div class="right-info">
          <p class="title" :title="item.title">
            <a :href="`/play/${item.id}?ep=${item.new_ep}`" target="_blank">{{ item.title }}</a>
          </p>
          <p class="update-ep">
            更新至
            <a :href="`/play/${item.id}?ep=${item.new_ep}`" target="_blank">{{ item.new_ep }}话</a>
          </p>
        </div>
      </li>
    </ul>
    ...
    <div class="toggle-block">
      <div class="toggle-btn" @click="showOrHide">
        ...
        <i class="icon" :class="showAll ? 'icon-arrow-up' : 'icon-arrow-down'"></i>
      </div>
    </div>
  </CommenItem>
</template>
